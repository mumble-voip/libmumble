...
// XCR krzmbrzl: Every type should be put into its own header file. If you really want to, you can then have this header
...
// Davide: Okay.
namespace mumble {
// XCR krzmbrzl: "Code" is a pretty bad name for this. To me this makes it sound as if this was some kind of function that can be
// executed. Instead, this should be called e.g. "ErrorCode".
//
// Davide: What about "EC"?
//
// XCR krzmbrzl: It seems strange to have this enum be defined here without context. Clearly these codes have some meaning in a given
...
//
// Davide: The naming is kept generic specifically to reuse the codes in different contexts.
// I wouldn't expect the library to become complex enough to justify the use of separate codes depending on the context...
...
struct Version {
	...
	// XCR krzmbrzl: I don't think "blob" is a good name. I would propose "encoded" or something similar, instead.
	// Davide: In theory "blob" specifically refers to multiple values encoded in a single variable.
	...
	// XCR krzmbrzl: The encoded Version representation shouldn't be exposed to the client. It should only have to deal with
	...
	//
	// Davide: Yeah. Also, encoding and decoding like this doesn't hit performance even when performed multiple times.
	...
	// XCR krzmbrzl: This shouldn't even be exposed to the library user, as we consider this to be deprecated
	// Davide: We actually have a specific namespace called "legacy", but for consistency let's hide this as well.
	...
	// XCR krzmbrzl: auto should not be used here
	// Davide: Why not?
...
// XCR krzmbrzl: We should type out the full name for publicly visible type definitions -> "Buffer"
// But even then this is a bit unspecific. It's not just any buffer, it's something specific -> "ByteBuffer"?
//
// Davide: The idea is to use "std::byte" for generic/binary buffers; basically when you don't know the underlying data format.
// As for "Buf" -> "Buffer", I'm not sure. I generally prefer full names too, but "Buf" is such a known term that I don't think it matters.
using Buf         = std::vector< std::byte >;
using BufRef      = boost::span< std::byte >;
using BufRefConst = boost::span< const std::byte >;

// XCR krzmbrzl: "FixedSize..." (otherwise it is not clear what exactly is fixed)
// Davide: I mean, the data can't certainly be fixed. The data type always is.
// I don't see any instances where the current name could be ambiguous, honestly.
template< size_t size > using FixedBuf = std::array< std::byte, size >;

// XCR krzmbrzl: The name of this function is too generic, imo. I would propose a more commonly used "toString", which still
// is very generic, but at least widely known.
//
// Davide: Across the codebase I used the "to" prefix when writing to another variable passed by the caller (e.g. toSockaddr() or toBuf() below).
// In this case I would suggest something like "codeText()", "errorText()", "errorCodeText()" or "ecText()".
...
// XCR krzmbrzl: Do we really need below functions in the public API? I don't think so.
// Davide: If I recall correctly they're only used in Pack, consider them convenience functions.
static constexpr std::byte toByte(const char byte) {
	return static_cast< std::byte >(byte);
}

// XCR krzmbrzl: "toByteBuffer"
// Davide: See my comment above (regarding the buffer naming).
